...
// TODO
exports.listSummaryModel = async ({ id, user_id }) => {
  const selectSummary = await selectSummaryDAO({ asset, user_id });
};

exports.updateSummaryModel = async ({
  asset,
  quantity,
  unitary_value,
  total,
    ...
    if (quantity === null || unitary_value === null) {
      ...
      if (verifySummary?.rowCount > 0) {
        ...
        const newAveragePrice =
          quantity > 0
            ? parseFloat(
                (parseFloat(parseFloat(unitary_value) * quantity) +
                  parseFloat(
                    parseFloat(verifySummary?.rows?.[0]?.average_price) *
                      verifySummary?.rows?.[0]?.quantity
                  )) /
                  newQuantity
              ).toFixed(2)
