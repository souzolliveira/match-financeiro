...
exports.listTransactionsModel = async ({
  start_date,
  end_date,
  group_by,
  user_id,
  ...
  if (listTransactions.rowCount > 0) {
    ...
    data = listTransactions.rows?.map((transaction) => {
      ...
      const total =
        parseFloat(transaction.total) + (parseFloat(transaction.result) || 0);
      if (transaction.transaction_type === transaction_types.INCOME)
        incomes += value;
      else if (transaction.transaction_type === transaction_types.EXPENSE) {
        expenses += value;
        if (transaction.costing === costing_types.FIXED)
          expenses_fixed += value;
        else if (transaction.costing === costing_types.VARIABLE)
          expenses_variable += value;
      } else if (transaction.transaction_type === transaction_types.INVESTIMENT)
        investiments += total;
      else if (transaction.transaction_type === transaction_types.REDEMPTION)
        redemptions += total;
      else if (transaction.transaction_type === transaction_types.DIVIDEND)
        dividends += value;
      return {
  ...
  balance = parseFloat(
    incomes - expenses - investiments + redemptions + dividends
  )
    .toFixed(2)
    .replace(".", ",");
  ...
  expenses_variable = parseFloat(expenses_variable)
    .toFixed(2)
    .replace(".", ",");
