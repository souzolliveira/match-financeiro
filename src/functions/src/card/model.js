...
const { selectCardByNameDAO, insertCardDAO, selectCardsDAO, updateCardDAO, deleteCardDAO } = require('./dao');

exports.createCardModel = async ({ name, expiration_day, payment_day, type, user_id }) => {
  ...
  if (!payment_day) {
    code = httpCode.BAD_REQUEST;
    message = 'É necessário informar o dia de pagamento/vencimento da fatura do cartão';
    return { code, message };
  }
...
exports.deleteCardModel = async ({ id, user_id }) => {
  ...
  if (verifyTransactions.rowCount > 0) {
    code = httpCode.BAD_REQUEST;
    message = 'Não é possível apagar o cartão, pois existem transações vinculadas a ele';
    return { code, message };
  }
