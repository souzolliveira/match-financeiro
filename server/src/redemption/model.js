...
const { selectSummaryDAO } = require('../summary/dao');
const { updateSummaryModel } = require('../summary/model');
const { insertRedemptionDAO, listRedemptionsDAO } = require('./dao');

exports.listRedemptionsModel = async ({ start_date, end_date, category, subcategory, asset, group_by, user_id }) => {
...
exports.createRedemptionModel = async ({ asset, quantity, unitary_value, total, result, redemption_date, observation, user_id }) => {
  let code = httpCode.ERROR;
  let message = httpMessage.ERROR;
  const date = now();
  ...
  const redemption = total !== null ? total : parseFloat(parseFloat(parseFloat(quantity) * parseFloat(unitary_value))).toFixed(2);

  const verifySummary = await selectSummaryDAO({ id: asset, user_id });
  if (verifySummary.rowCount > 0) {
    if (!verifyAsset.rows[0].quantifiable && verifySummary.rows[0].total < redemption) {
      code = httpCode.BAD_REQUEST;
      message = 'É necessário informar um valor de resgate menor do que o valor já investido';
      return { code, message };
    }
    if (verifyAsset.rows[0].quantifiable && verifySummary.rows[0].quantity < quantity) {
      code = httpCode.BAD_REQUEST;
      message = 'É necessário informar uma quantidade de cotas para resgate menor do que a quantidade de cotas já compradas';
      return { code, message };
    }
  }
